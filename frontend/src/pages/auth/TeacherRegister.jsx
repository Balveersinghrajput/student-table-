...
export default function TeacherRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const data = await authService.teacherRegister(form.name, form.email, form.password);
      teacherLogin(data.token, data.teacher);
      toast.success("Welcome, " + data.teacher.name + "! Registration successful.");
      navigate("/dashboard");
    } catch (err) {
    ...
    <div style={{ minHeight: "100vh", background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div className="fade-in" style={{ width: "100%", maxWidth: 400, background: "#fff", borderRadius: 14, padding: 36, boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <h1 style={{ color: "#1e293b", fontSize: 22, fontWeight: 700, marginBottom: 4 }}>Teacher Registration</h1>
          <p style={{ color: "#64748b", fontSize: 13 }}>Create an account to manage your students</p>
        </div>

        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            <label style={{ color: "#64748b", fontSize: 11, fontWeight: 600, textTransform: "uppercase", display: "block", marginBottom: 4 }}>Full Name</label>
            <input className={"input" + (errors.name ? " input-error" : "")} type="text" placeholder="John Doe" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} style={{ width: "100%" }} />
          ...
          <div>
            <label style={{ color: "#64748b", fontSize: 11, fontWeight: 600, textTransform: "uppercase", display: "block", marginBottom: 4 }}>Confirm Password</label>
            <input className={"input" + (errors.confirmPassword ? " input-error" : "")} type="password" placeholder="••••••••" value={form.confirmPassword} onChange={(e) => setForm({ ...form, confirmPassword: e.target.value })} style={{ width: "100%" }} />
            {errors.confirmPassword && <p style={{ color: "#ef4444", fontSize: 11, marginTop: 2 }}>{errors.confirmPassword}</p>}
          </div>
        ...
        <p style={{ textAlign: "center", marginTop: 16, color: "#64748b", fontSize: 12 }}>
          Already have an account? <Link to="/teacher-login" style={{ color: "#8b5cf6", textDecoration: "none", fontWeight: 600 }}>Sign In →</Link>
        </p>
